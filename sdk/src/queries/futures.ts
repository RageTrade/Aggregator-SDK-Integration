...
export const queryAccountsFromSubgraph = async (sdk: KwentaSDK, walletAddress: string | null): Promise<string[]> => {
...
export const queryCrossMarginAccounts = async (sdk: KwentaSDK, walletAddress: string): Promise<string[]> => {
  // TODO: Contract should be updating to support one to many
  const accounts = await sdk.context.contracts.SmartMarginAccountFactory?.getAccountsOwnedBy(walletAddress)
  return accounts ?? []
}
...
export const queryFuturesTrades = (sdk: KwentaSDK, marketKey: FuturesMarketKey, minTs: number, maxTs: number) => {
  return getFuturesTrades(
    sdk.futures.futuresGqlEndpoint,
    {
...
export const queryFundingRateHistory = async (
  ...
  const response: any = await request(
    sdk.futures.futuresGqlEndpoint,
    gql`
      query fundingRateUpdate($marketAsset: Bytes!, $minTimestamp: BigInt!, $period: FundingRatePeriodType!) {
        fundingRatePeriods(where: { asset: $marketAsset, timestamp_gt: $minTimestamp, period: $period }, first: 1000) {
          timestamp
          fundingRate
        }
      }

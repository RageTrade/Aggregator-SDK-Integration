...
export default class KwentaTokenService {
  ...
  public changePoolTokens(amount: string, action: 'stake' | 'withdraw') {
    ...
    return this.sdk.transactions.createContractTxn(this.sdk.context.contracts.StakingRewards, action, [
      wei(amount).toBN()
    ])
  }
  ...
  public async getEscrowData() {
    ...
    const schedules = await RewardEscrow.getVestingSchedules(walletAddress, 0, DEFAULT_NUMBER_OF_FUTURES_FEE)
  ...
  public async getEscrowV2Data() {
    ...
    const schedules = await RewardEscrowV2.getVestingSchedules(walletAddress, 0, DEFAULT_NUMBER_OF_FUTURES_FEE)
  ...
  public approveToken(token: ContractName, spender?: ContractName, amount = ethers.constants.MaxUint256) {
    ...
    return this.sdk.transactions.createContractTxn(tokenContract, 'approve', [spenderAddress, amount])
  }

  public redeemToken(token: ContractName, options: { hasAddress: boolean } = { hasAddress: false }) {
    const tokenContract = this.sdk.context.contracts[token]
  ...
  public async getClaimableAllRewards(
    ...
    const fileNames = adjustedPeriods.map(
      (i) =>
        `${this.sdk.context.networkId === 420 ? `goerli-` : ''}epoch-${isSnx ? i - OP_REWARDS_CUTOFF_EPOCH : i}${
          isOp ? (isSnx ? '-snx-op' : '-op') : ''
        }.json`
    )
  ...
  private performStakeAction(
    ...
    const { RewardEscrow, RewardEscrowV2, KwentaStakingRewards, KwentaStakingRewardsV2 } = this.sdk.context.contracts
    ...
    const contract =
      options?.version === 1 ? (options?.escrow ? RewardEscrow : KwentaStakingRewards) : KwentaStakingRewardsV2

    return this.sdk.transactions.createContractTxn(contract, `${action}${options?.escrow ? 'Escrow' : ''}`, [amount])
  }
}

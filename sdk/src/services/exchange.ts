...
export default class ExchangeService {
  ...
  public getTxProvider(baseCurrencyKey: string, quoteCurrencyKey: string) {
    ...
    if (this.synthsMap?.[baseCurrencyKey as SynthSymbol] && this.synthsMap?.[quoteCurrencyKey as SynthSymbol])
      return 'synthetix'
  ...
  public async getSlippagePercent(
    ...
    if (txProvider === '1inch') {
      const { quoteTradePrice: totalTradePrice, baseTradePrice: estimatedBaseTradePrice } = await this.getTradePrices(
        txProvider,
        quoteCurrencyKey,
        baseCurrencyKey,
        quoteAmountWei,
  ...
  public async swapSynthSwap(
    ...
    const params = await this.getOneInchSwapParams(oneInchFrom, oneInchTo, synthAmountEth, fromToken.decimals)
    ...
    if (this.tokensMap[toToken.symbol]) {
      ...
      if (formattedData.functionSelector === 'swap') {
        return metaOnly
          ...
          : this.sdk.transactions.createContractTxn(SynthSwap, 'swapInto', [symbolBytes, formattedData.data])
      } else {
        return metaOnly
  ...
  public async swapOneInchMeta(
    ...
    const params = await this.getOneInchSwapParams(quoteTokenAddress, baseTokenAddress, amount, quoteDecimals)
  ...
  public async swapOneInch(quoteTokenAddress: string, baseTokenAddress: string, amount: string, quoteDecimals: number) {
    const params = await this.getOneInchSwapParams(quoteTokenAddress, baseTokenAddress, amount, quoteDecimals)
  ...
  public async approveSwap(quoteCurrencyKey: string, baseCurrencyKey: string) {
    ...
    if (quoteCurrencyContract) {
      const { hash } = await this.sdk.transactions.createContractTxn(quoteCurrencyContract, 'approve', [
        approveAddress,
  ...
  public async handleExchange(
    ...
    if (txProvider === '1inch' && !!this.tokensMap) {
      tx = await this.swapOneInch(quoteCurrencyTokenAddress, baseCurrencyTokenAddress, quoteAmount, quoteDecimals)
      ...
      const shouldExchange =
        !!exchangeParams && !!this.sdk.context.walletAddress && !!this.sdk.context.contracts.Synthetix
  ...
  public async getTransactionFee(
    ...
    if (txProvider === 'synthswap' || txProvider === '1inch') {
      const gasInfo = await this.getGasEstimateForExchange(txProvider, quoteCurrencyKey, baseCurrencyKey, quoteAmount)
  ...
  public async getOneInchQuote(baseCurrencyKey: string, quoteCurrencyKey: string, amount: string) {
    ...
    if (txProvider === '1inch') {
      const estimatedAmount = await this.quoteOneInch(quoteTokenAddress, baseTokenAddress, amount, decimals)
    ...
    if (this.tokensMap[quoteCurrencyKey]) {
      ...
      const estimatedAmount = await this.quoteOneInch(sUSD.address, baseTokenAddress, usdAmount.toString(), decimals)

      return estimatedAmount
    } else {
      const estimatedAmount = await this.quoteOneInch(quoteTokenAddress, sUSD.address, amount, decimals)
  ...
  public validCurrencyKeys(quoteCurrencyKey?: string, baseCurrencyKey?: string) {
    return [quoteCurrencyKey, baseCurrencyKey].map((currencyKey) => {
      return !!currencyKey && (!!this.synthsMap[currencyKey as SynthSymbol] || !!this.tokensMap[currencyKey])
    })
  }
  ...
  private getExchangeParams(
    ...
    if (isAtomic) {
      return [sourceCurrencyKey, sourceAmountBN, destinationCurrencyKey, RAGE_TRACKING_CODE, minAmount.toBN()]
    } else {
      return [
        sourceCurrencyKey,
        sourceAmountBN,
  ...
  public async getSynthSuspensions() {
    ...
    const responses = (await this.sdk.context.multicallProvider.all(calls)) as [boolean, BigNumber][]

    let ret: Record<string, { isSuspended: boolean; reasonCode: number; reason: SynthSuspensionReason | null }> = {}
    let i = 0
  ...
  private async quoteOneInch(quoteTokenAddress: string, baseTokenAddress: string, amount: string, decimals: number) {
    ...
    return ethers.utils.formatUnits(response.data.toTokenAmount, response.data.toToken.decimals).toString()
  }
  ...
  private async getPairRates(quoteCurrencyKey: string, baseCurrencyKey: string) {
    return this.checkIsAtomic(baseCurrencyKey, quoteCurrencyKey)
      ? await Promise.all([this.getAtomicRates(quoteCurrencyKey), this.getAtomicRates(baseCurrencyKey)])
  ...
  private async getOneInchApproveAddress() {
    const response = await axios.get<OneInchApproveSpenderResponse>(this.oneInchApiUrl + 'approve/spender')
  ...
  private getCoingeckoPricesForCurrencies(coingeckoPrices: PriceResponse | null, baseAddress: string | null) {
  ...
  private getExchangeRatesForCurrencies(
    ...
    return !rates || !base || !quote || !rates[base] || !rates[quote] ? wei(0) : rates[base].div(rates[quote])
  }

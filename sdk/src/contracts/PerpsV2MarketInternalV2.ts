...
class FuturesMarketInternal {
  ...
  getTradePreview = async (account: string, sizeDelta: BigNumber, marginDelta: BigNumber, tradePrice: BigNumber) => {
  ...
  _orderFee = async (tradeParams: TradeParams) => {
    ...
    if (this._sameSide(marketSkew.add(tradeParams.sizeDelta), marketSkew)) {
      const staticRate = this._sameSide(notionalDiff, marketSkew) ? tradeParams.takerFee : tradeParams.makerFee
  ...
  _recomputeMarginWithDelta = async (position: Position, price: BigNumber, marginDelta: BigNumber) => {
  ...
  _netFundingPerUnit = async (startIndex: BigNumber, price: BigNumber) => {
    const fundingSequenceVal = await this._perpsV2MarketContract.fundingSequence(startIndex.toNumber())
  ...
  _maxLeverageForSize = async (size: BigNumber) => {
    ...
    return liqBuffer.div(wei(size).abs().div(skewScaleWei).mul(liqPremMultiplierWei).add(liqBufferRatioWei)).toBN()
  }

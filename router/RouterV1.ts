...
import { Protocol, MarketInfo, DynamicMarketMetadata, CreateOrder, UnsignedTxWithMetadata, UpdateOrder, CancelOrder, PositionInfo, ClosePositionData, UpdatePositionMarginData, CollateralData, AmountInfo, PageOptions, PaginatedRes, OrderInfo, HistoricalTradeInfo, LiquidationInfo, OpenTradePreviewInfo, CloseTradePreviewInfo, PreviewInfo, IRouterAdapterBaseV1, ProtocolId, Market, RouterAdapterMethod, PositionData } from "../src/interfaces/V1/IRouterAdapterBaseV1";
...
class RouterV1 implements IRouterV1 {
    ...
    async closePosition(positionInfo: PositionInfo[], closePositionData: ClosePositionData[]): Promise<UnsignedTxWithMetadata[]> {
    ...
    async updatePositionMargin(positionInfo: PositionInfo[], updatePositionMarginData: UpdatePositionMarginData[]): Promise<UnsignedTxWithMetadata[]> {
        const promises: Promise<UnsignedTxWithMetadata[]>[] = [];
        positionInfo.forEach((position, index) => {
            const protocolId = this._checkAndGetProtocolId(position.marketId);
    ...
    async getAllOrdersForPosition(wallet: string, positionInfo: PositionInfo[], pageOptions: PageOptions | undefined): Promise<PaginatedRes<Record<PositionData['posId'], OrderInfo[]>>> {
        const promises: Promise<PaginatedRes<Record<PositionData['posId'], OrderInfo[]>>>[] = [];
        const result: Record<PositionData['posId'], OrderInfo[]>[] = []

        for (const position of positionInfo) {
            const protocolId = this._checkAndGetProtocolId(position.marketId);
            promises.push(this.adapters[protocolId].getAllOrdersForPosition(wallet, [position], undefined));
        }
    ...
    async getOpenTradePreview(wallet: string, orderData: CreateOrder[], existingPos: (PositionInfo | undefined)[]): Promise<OpenTradePreviewInfo[]> {
    ...
    async getCloseTradePreview(wallet: string, positionInfo: PositionInfo[], closePositionData: ClosePositionData[]): Promise<CloseTradePreviewInfo[]> {
    ...
    async getUpdateMarginPreview(wallet: string, marketIds: Market['marketId'][], isDeposit: boolean[], marginDelta: AmountInfo[], existingPos: (PositionInfo | undefined)[]): Promise<PreviewInfo[]> {
        const promises: Promise<PreviewInfo[]>[] = [];
        marketIds.forEach((marketId, index) => {
            const protocolId = this._checkAndGetProtocolId(marketId);
            promises.push(this.adapters[protocolId].getUpdateMarginPreview(wallet, [marketId], [isDeposit[index]], [marginDelta[index]], [existingPos[index]]))
        })
        const out = await Promise.all(promises);
        return out.flat();
    }

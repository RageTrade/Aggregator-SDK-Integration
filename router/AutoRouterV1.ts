...
import { AmountInfo, AmountInfoInToken, ApiOpts, CreateOrder, DynamicMarketMetadata, MarketInfo, OpenTradePreviewInfo, PositionInfo, TradeDirection } from "../src/interfaces/V1/IRouterAdapterBaseV1";
import RouterV1 from "./RouterV1";
import { MarketTag, MarketWithMetadata, MarketWithPreview, RouteData, TokenWithPrice, getBestFundingReduceCallback, getReduceCallback } from "./Route";
...
export default class AutoRouterV1 extends RouterV1 {
    ...
    private _logMarkets(markets: MarketInfo[]) {
        markets.forEach((market) => {
            console.log({marketId: market.marketId, indexToken: market.indexToken.symbol, longCollateral: market.longCollateral.map((token) => token.symbol), shortCollateral: market.shortCollateral.map((token) => token.symbol)})
        })
    }
    private async _getEligibleMarkets(indexToken: Token, collateralTokenWithPriceList: TokenWithPrice[], direction: TradeDirection): Promise<MarketInfo[]> {
        ...
        const eligibleMarkets = markets.filter((market) => {
            ...
            const marketSymbols = direction == 'LONG' ? market.longCollateral.map((token) => token.symbol): market.shortCollateral.map((token) => token.symbol)
            const correctCollateral = marketSymbols.some((marketSymbol) => collateralTokenSymbols.includes(marketSymbol))
    ...
    private _getCreateOrder(adapter: IAdapterV1, market: MarketInfo, routeData: RouteData, collateralTokenWithPrice: TokenWithPrice): CreateOrder {
        const amountInfoInToken = adapter.getAmountInfoType()
        let sizeDeltaAmountInfo:AmountInfo = amountInfoInToken.sizeDeltaInToken? {isTokenAmount:true, amount:routeData.sizeDeltaToken} : {isTokenAmount:false, amount:routeData.sizeDeltaUSD}
        
        const marginDeltaToken = divFN(routeData.marginDeltaUSD, collateralTokenWithPrice.price, collateralTokenWithPrice.token.decimals)
        let marginDeltaAmountInfo:AmountInfo = amountInfoInToken.collateralDeltaInToken? {isTokenAmount:true, amount:marginDeltaToken} : {isTokenAmount:false, amount:routeData.marginDeltaUSD}
        ...
        const order: CreateOrder = {
            ...
            triggerData: { triggerPrice: FixedNumber.fromValue(0,30), triggerAboveThreshold:true },
            slippage: undefined,
            type: 'MARKET'
        }
        return order
    ...
    async _getDynamicMarketMetadata(eligibleMarkets: MarketInfo[], opts?: ApiOpts | undefined): Promise<MarketWithMetadata[]> {
    ...
    async _getTradePreview(
        ...
        routeData.collateralTokens.forEach((tokenWithPrice) => collateralTokenSymbolMap.set(tokenWithPrice.token.symbol, tokenWithPrice))
        ...
        eligibleMarkets.forEach((market) => {
            ...
            const marketCollateralToken = marketCollateralTokens.find((token) => collateralTokenSymbols.includes(token.symbol))
            if(!marketCollateralToken) return
            const order = this._getCreateOrder(adapter, market, routeData, collateralTokenSymbolMap.get(marketCollateralToken.symbol)!)
    ...
    async getMarketTags(
        ...
        const eligibleMarkets = await this._getEligibleMarkets(routeData.indexToken, routeData.collateralTokens, routeData.direction);
